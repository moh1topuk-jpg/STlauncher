...
public class ModManagerReplaceTests
{
    ...
    [Fact]
    public void DisableOtherVersions_SwitchesOffTheOlderJarWithTheSameId()
    {
        var game = TempGame();
        ...
        var removed = Manager().DisableOtherVersions(game, "sodium-fabric-0.8.12+mc1.21.11.jar");
        ...
        Assert.True(File.Exists(Path.Combine(game, "mods", "sodium-fabric-0.8.2+mc1.21.11.jar.disabled")));
    ...
    [Fact]
    public void DisableOtherVersions_LeavesDisabledCopiesAndUnknownJars()
    {
        var game = TempGame();
        ...
        var removed = Manager().DisableOtherVersions(game, "sodium-new.jar");
