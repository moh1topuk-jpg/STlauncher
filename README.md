...
работает, даже если GitHub у вас не открывается.
...
**Windows пишет «Система Windows защитила ваш компьютер».** Лаунчер не подписан платным
сертификатом, поэтому SmartScreen его не знает. Нажмите «Подробнее» → «Выполнить в любом
случае». Проверить, что файл настоящий, можно так — см. [ниже](#проверка-подлинности).
...
- **Переход из других лаунчеров.** Лаунчер находит готовые сборки TLauncher, официального
  лаунчера, Prism, MultiMC, CurseForge, Modrinth App, GDLauncher и ATLauncher — или в
  любой указанной папке. Сборку можно подключить на месте или скопировать к себе.
