...
Каждый инстанс — отдельная сборка со своей папкой в `instances/`: версия игры,
загрузчик, память, разрешение, дополнительные аргументы Minecraft, набор модов
из каталога и собственный `servers.dat`.
Параметры хранятся в `instances/<id>/instance.json`. Старые настройки из
`settings.json` автоматически переносятся в сборку «Default» при первом запуске
новой версии.
