...
Страница **Настройки** → *Автоматические бэкапы*: включение, интервал, максимальное
число и лимит суммарного размера. В архив попадают `saves`, `config`, `mods`,
`resourcepacks`, `shaderpacks`, `options.txt` и `servers.dat`. Есть кнопка ручного
бэкапа. Лишние копии удаляются по количеству и по размеру, самая свежая не удаляется
никогда.
