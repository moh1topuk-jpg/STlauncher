...
Те же числа, нарисованные: `https://showtime-stats.moh1topuk.workers.dev/dashboard` —
пользователи и запуски, исходы проверок обновлений, онлайн сервера за сутки и за неделю.
Страница обновляется сама раз в минуту. Её исходник — [`dashboard.html`](../workers/stats/dashboard.html);
после правки его нужно снова вложить в `worker.js` (константа `DASHBOARD_HTML`) и задеплоить воркер.
